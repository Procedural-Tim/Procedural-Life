...
const { goodOrEvilValues, lawOrChaosValues } = require("../Data/alignments")
...
function mapRelationship(srcId, extId, srcDep, extDep) {
    const [, 
        ...
        , 
        ,
        srcRelationshipStatus
    ] = srcDep

    if ([relationshipStatusValues.NA, relationshipStatusValues.SINGLE, relationshipStatusValues.WIDOWED].includes(srcRelationshipStatus)) {
        return ["NA", undefined]
    }
...
function genderPasses(srcGender, extGender, srcOrientation, extOrientation) {
    ...
    if (srcOrientation === sexualOrientationValues.HETEROSEXUAL) {
        if (srcGender === genderValues.FEMALE && extGender === genderValues.FEMALE) {
            return false
        }
    ...
    if (extOrientation === sexualOrientationValues.HETEROSEXUAL) {
        if (extGender === genderValues.FEMALE && srcGender === genderValues.FEMALE) {
            return false
        }
...
function alignmentPasses(srcGE, extGE, srcLC, extLC) {
    ...
    const chaotic = [lawOrChaosValues.C3, lawOrChaosValues.C2, lawOrChaosValues.C1]
    let matchScore = 0
    ...
    return getWeightedRandomValue([{ value: true, weight: matchScore}, {value: false, weight: 8 - matchScore}])
}

function filterRelationship(srcDependencyValues, extDependencyValues) {
    const [srcGender, 
        srcAge, 
        srcOrientation, 
        srcGE, 
    ...
    const [extGender, extAge, extOrientation, extGE,  extLC,  extRace, extRelationshipStatus] = extDependencyValues

    // Under 18 always has NA so we don't have to check it again
    if ([relationshipStatusValues.NA, relationshipStatusValues.SINGLE, relationshipStatusValues.WIDOWED].includes(extRelationshipStatus)) {
        return false
    }
    ...
    if (srcRace !== extRace) {
        ...
        if (!getWeightedRandomValue([{ value: true, weight: 1}, { value: false, weight: 2}])) {
            return false
        }
    }
